...
class EntityOperation<T> {
	...
	private Map<String, FieldMapping> getFieldMappings(Map<String, Method> getters, Map<String, Method> setters, Field[] fields) {
		if(getters.size() == 0){ System.err.println("没有扫描到任何getter"); }
		if(setters.size() == 0){ System.err.println("没有扫描到任何setters"); }
        ...
        if(mappings.size() == 0){
            System.err.println("没有扫描到任何映射关系");
        }
        return mappings;
    }
